...
function testRoundTrip(inputs: unknown[]) {
    for (const input of inputs) {
        ...
        const json = typeof input === "bigint" ? input.toString() : JSON.stringify(input);
        if (json.length > 60) {
...
function dumpBuf(buf: Uint8Array, width = 10): string {
    const chars = Array.from(buf).map((v) => v >= 32 && v < 127 ? String.fromCharCode(v) : ".").join("");
    return `${chars.padEnd(width, " ")}   |${Array.from(buf).map(v => v.toString(16).padStart(2, "0")).join("")}|`;
}